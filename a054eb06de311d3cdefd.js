...
function setHeaderBanner(imgSource, team) {
...
function teamColorsPage(e) {
    ...
    // let seasonNum = "01";
    let seasonNum = e.target.dataset.seasonNum;
    let teamLogoSrc = teams_eachTeamObjectMAP.get(team)[`S0${seasonNum}HomeFilePath`];
    let homeColorScheme = teams_eachTeamObjectMAP.get(team)[`S0${seasonNum}Home`];
    let awayTeamLogo = teams_eachTeamObjectMAP.get(team)[`S0${seasonNum}AwayFilePath`];
...
;// CONCATENATED MODULE: ./src/scripts/layouts/setMainNavbar.js
// this function is run in listeners.js
  ...
  function setMainNavbar(season) {
    ...
    if (teamsThisSeason > 5 && screenWidth < 500) {
      ...
      for (let i = 0; i < teamsThisSeason / 2; i++) {
        // first row of teams
        let imageSource = teams_eachTeamObjectMAP.get(
          teams_eachSeasonsTeamsMAP.get(season)[i]
  ...
  function setLayout(imageSource, element, season, id) {
    ...
    element += `<img 
        src="${image}" 
        alt="${teams_eachSeasonsTeamsMAP.get(season)[id]}" 
        data-team-name="${teams_eachSeasonsTeamsMAP.get(season)[id]}" 
...
/******/ })()
;
//# sourceMappingURL=a054eb06de311d3cdefd.js.map